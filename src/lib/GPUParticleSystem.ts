 ...
 *
 * Shader and javascript packing code derived from several Stack Overflow examples.
 *
 */
...
export class GPUParticleSystem extends THREE.Object3D {
    ...
    constructor(options: ParticleSystemOptions) {
        ...
        options = options || ({} as ParticleSystemOptions);
...
export class GPUParticleContainer extends THREE.Object3D {
    ...
    constructor(maxParticles: number, particleSystem: GPUParticleSystem) {
        ...
        this.spawnParticle = function (options) {
            ...
            var startTimeAttribute = this.particleShaderGeo.getAttribute(
                "startTime"
            );
            var velocityAttribute = this.particleShaderGeo.getAttribute(
                "velocity"
            );
            ...
            var lifeTimeAttribute = this.particleShaderGeo.getAttribute(
                "lifeTime"
            );
            ...
            var velocityRandomness =
                options.velocityRandomness !== undefined
                    ...
                    : 0;
            var colorRandomness =
                options.colorRandomness !== undefined
                    ? options.colorRandomness
                    : 1;
            var turbulence =
                options.turbulence !== undefined ? options.turbulence : 1;
            var lifetime =
                options.lifetime !== undefined ? options.lifetime : 5;
            var size = options.size !== undefined ? options.size : 10;
            var sizeRandomness =
                options.sizeRandomness !== undefined
                    ? options.sizeRandomness
                    : 0;
            var smoothPosition =
                options.smoothPosition !== undefined
            ...
            var velX =
                velocity.x + particleSystem.random() * velocityRandomness;
            var velY =
                velocity.y + particleSystem.random() * velocityRandomness;
            var velZ =
                velocity.z + particleSystem.random() * velocityRandomness;

            velX = THREE.MathUtils.clamp(
                (velX - -maxVel) / (maxVel - -maxVel),
                0,
                1
            );
            velY = THREE.MathUtils.clamp(
                (velY - -maxVel) / (maxVel - -maxVel),
                0,
                1
            );
            velZ = THREE.MathUtils.clamp(
                (velZ - -maxVel) / (maxVel - -maxVel),
                0,
                1
            );
            ...
            sizeAttribute.array[i] =
                size + particleSystem.random() * sizeRandomness;
            lifeTimeAttribute.array[i] = lifetime;
            startTimeAttribute.array[i] =
                this.time + particleSystem.random() * 2e-2;
        ...
        this.geometryUpdate = function () {
            if (this.particleUpdate === true) {
                ...
                var colorAttribute = this.particleShaderGeo.getAttribute(
                    "color"
                );
                var sizeAttribute = this.particleShaderGeo.getAttribute("size");
                var lifeTimeAttribute = this.particleShaderGeo.getAttribute(
                ...
                if (this.offset + this.count < this.PARTICLE_COUNT) {
                    ...
                    sizeAttribute.updateRange.offset =
                        this.offset * sizeAttribute.itemSize;
                    ...
                    colorAttribute.updateRange.count =
                        this.count * colorAttribute.itemSize;
                    sizeAttribute.updateRange.count =
                        this.count * sizeAttribute.itemSize;
...
function randomGenerator() {
    ...
    return function () {
        return ++i >= rand.length ? rand[(i = 1)] : rand[i];
    };
}
